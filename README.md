...
- **Form dạng bảng có ô phụ thuộc** (`lib-material-table-form`, mat-table): ô trong `<td>` không có label, nhãn giờ lấy từ `<th>` cùng cột (qua class `mat-column-X`, hoặc theo vị trí cột), bỏ dấu `*`. Các ô còn `disabled` cho tới khi chọn xong ô đầu (chọn "Loại chứng từ" mới mở "Số định danh", "Ngày cấp"…) trước đây bị bỏ qua vì lúc quét chúng đang khoá. Giờ sau khi điền xong một lượt, extension **quét lại**: ô nào vừa được mở khoá và còn trống thì điền tiếp (chế độ AI hỏi thêm một lần chỉ cho những ô đó, chế độ Ngẫu nhiên sinh tại chỗ), tối đa 2 lượt bổ sung. Panel ghi "Lượt 2: N ô vừa được mở khoá". Tắt trong *Cài đặt → Hành vi* nếu không muốn.
- **Ngày theo ngữ nghĩa**: dữ liệu thử chọn "Ngày cấp" / "bắt đầu" trong quá khứ, "Ngày hết hạn" / "kết thúc" trong tương lai, ngày sinh 1980–2005, và luôn nằm trong `min`/`max` của input. "Số định danh" / "số giấy tờ" → 12 chữ số; "Nơi cấp" → tên cơ quan công an.
